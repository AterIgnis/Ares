...
_loadPrevious = [_this, 2, true] call BIS_fnc_param;
...
_titleVariableIdentifier = format ["Ares_ChooseDialog_DefaultValues_%1", [_titleText, " ", "_"] call KRON_Replace];
{
	...
	if (_loadPrevious && _titleText != "") then
	{
	...
	if (count _choices == 0) then 
	{
		...
		_choiceEdit ctrlSetText format ["%1",_defaultChoice];
		...
		_choiceEdit ctrlCommit 0;
	        ...
	        if (typeName _defaultChoice != typeName 0 || _defaultChoice >= count _choices) then {
	        	_defaultChoice = 0;
	        };
		...
		if (isNil "Ares_CustomDialog_Set_Value_Function") then {
			Ares_CustomDialog_Set_Value_Function = {
				...
				if (_id >= _max) then {
					_id = _max - 1;
					_combo lbSetCurSel _id;
				};
				missionNamespace setVariable [_var, _id];
				true;
			};
		};
		_onChange = format ["[_this select 0, _this select 1, '%1', %2] call Ares_CustomDialog_Set_Value_Function;", format["Ares_ChooseDialog_ReturnValue_%1", _forEachIndex], count _choices];
		_choiceCombo ctrlSetEventHandler ["LBSelChanged", _onChange];
		_choiceCombo ctrlCommit 0;
	};
...
if (missionNamespace getVariable "Ares_ChooseDialog_Result" == 1) then
{
	_returnValue = [];
	{
	        _val = missionNamespace getVariable (format["Ares_ChooseDialog_ReturnValue_%1",_forEachIndex]);
		_returnValue set [_forEachIndex, _val];
	} forEach _choicesArray;
