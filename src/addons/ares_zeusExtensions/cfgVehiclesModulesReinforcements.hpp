class Ares_Module_Reinforcements_Create_Lz : Ares_Reinforcements_Module_Base
{
	scopeCurator = 2;
	displayName = "Create New LZ";
	function = "Ares_fnc_ReinforcementsCreateLz";
	icon = "\ares_zeusExtensions\data\icon_lz.paa";
	portrait = "\ares_zeusExtensions\data\icon_lz.paa";
};

class Ares_Module_Reinforcements_Create_Rp : Ares_Reinforcements_Module_Base
{
	scopeCurator = 2;
	displayName = "Create New RP";
	function = "Ares_fnc_ReinforcementsCreateRp";
	icon = "\ares_zeusExtensions\data\icon_rp.paa";
	portrait = "\ares_zeusExtensions\data\icon_rp.paa";
};

class Ares_Module_Reinforcements_Spawn_Units : Ares_Reinforcements_Module_Base
{
	scopeCurator = 2;
	displayName = "Spawn Units";
	function = "Ares_fnc_ReinforcementsCreateUnits";
};

class Ares_Module_Reinforcements_Spawn_Garrison : Ares_Reinforcements_Module_Base
{
	scopeCurator = 2;
	displayName = "Spawn Garrison";
	function = "Ares_fnc_SpawnGarrison";
};
